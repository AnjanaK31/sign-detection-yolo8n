...
def debug_yolo_lines():
    ...
    print(f"Total foreground pixels: {np.sum(binary_foreground > 0)}")
    print(f"Total yolo_mask pixels: {np.sum(yolo_mask > 0)}")
    print(f"Total big_lines_mask pixels: {np.sum(big_lines_mask > 0)}")
    print(f"Total red_mask pixels: {np.sum(red_mask > 0)}")
    
    # Save the masks to see what they look like
    cv2.imwrite("d:/Internship/OCR_PDF/internt-ocrmodel/scratch/debug_red_mask.png", red_mask)
    print("Saved debug_red_mask.png")
