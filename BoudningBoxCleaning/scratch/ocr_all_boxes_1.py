...
def inspect_1():
    ...
    inp = "d:/Internship/OCR_PDF/sign-detection-yolo8n/toTest/1.jpeg"
    img = cv2.imread(inp)
    H, W, C = img.shape
    ...
    if result.obb is None or len(result.obb) == 0:
        print("No boxes found in 1.jpeg")
        return
        
    reader = easyocr.Reader(['en'])
    xywhr = result.obb.xywhr.cpu().numpy()
    print(f"Found {len(xywhr)} bounding boxes in 1.jpeg")
    ...
    for idx, (cx, cy, w, h, r) in enumerate(xywhr):
        ...
        # Save all crops to a temporary folder in scratch to inspect them if needed
        out_path = f"d:/Internship/OCR_PDF/BoudningBoxCleaning/scratch/1_crop_{idx}.png"
