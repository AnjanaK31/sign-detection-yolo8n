...
def inspect_eval():
    ...
    inp = "d:/Internship/OCR_PDF/internt-ocrmodel/eval_output/eval_page.png"
    img = cv2.imread(inp)
    H, W, C = img.shape
    ...
    if result.obb is None or len(result.obb) == 0:
        print("No boxes found in eval_page.png")
        return
        
    reader = easyocr.Reader(['en'])
    xywhr = result.obb.xywhr.cpu().numpy()
    print(f"Found {len(xywhr)} bounding boxes in eval_page.png")
    ...
    for idx, (cx, cy, w, h, r) in enumerate(xywhr):
        ...
        out_path = f"d:/Internship/OCR_PDF/BoudningBoxCleaning/scratch/eval_crop_{idx}.png"
