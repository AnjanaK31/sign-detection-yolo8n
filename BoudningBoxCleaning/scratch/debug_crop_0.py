...
if img_bgr is None:
    print(f"Failed to load backup {img_path}")
    sys.exit(1)
...
# Run standard line cleaning and output intermediate debug info
# We'll print details of what clean_crop_lines does
# Note: we can modify a temporary script or just run it and print shapes
...
print(f"Number of erased pixels: {len(erased_pts)}")

out_dir = r"d:\Internship\OCR_PDF\BoudningBoxCleaning\scratch"
cv2.imwrite(os.path.join(out_dir, "crop_0_orig.png"), img_bgr)
cv2.imwrite(os.path.join(out_dir, "crop_0_cleaned.png"), cv2.cvtColor(np.array(cleaned), cv2.COLOR_RGB2BGR))
cv2.imwrite(os.path.join(out_dir, "crop_0_review.png"), cv2.cvtColor(review, cv2.COLOR_RGB2BGR))

# Let's save the intermediate masks
cv2.imwrite(os.path.join(out_dir, "crop_0_mser_char.png"), mser_char_mask)
cv2.imwrite(os.path.join(out_dir, "crop_0_mser_line.png"), mser_line_mask)
cv2.imwrite(os.path.join(out_dir, "crop_0_small_islands.png"), small_islands)
cv2.imwrite(os.path.join(out_dir, "crop_0_large_islands.png"), large_islands)
print("Saved debug outputs to scratch.")
